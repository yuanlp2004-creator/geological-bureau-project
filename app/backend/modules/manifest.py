"""静态定义模块、权限、审计、能力和导航契约。

本文件同时是 ``/api/v1/capabilities`` 与生成文件
``app/manifest.generated.json`` 的来源；前端消费该契约，不再硬编码第二份模块列表。
"""
...
METHOD_MANIFEST = ModuleManifest(
    ...
    navigation_entries=(
        nav("methods.lifecycle", "methods", "", "方法管理与参数", "选择、管理方法与编辑方法参数", "methods", 10, ("methods.read",), view="conditions"),
        nav("methods.lines", "methods", "", "分析谱线", "谱线、内标、峰值、拟合与标准点", "methods", 20, ("methods.read",), view="lines"),
        nav("methods.print-preview", "methods", "", "预览与打印", "页面、打印机设置与方法预览输出", "methods", 30, ("methods.read",), view="print-preview"),
    ),
)
...
LEGACY_MIGRATION_MANIFEST = ModuleManifest(
    ...
    capabilities=("legacy-source-discovery", "legacy-directory-dialog", "jet-access-win-x86", "read-only-staging", "atomic-commit", "idempotent-import"),
    navigation_entries=(
...
SPECTRUM_MIGRATION_MANIFEST = ModuleManifest(
    ...
    dependencies=("core", "auth"), capabilities=("legacy-source-discovery", "legacy-directory-dialog", "cdt-cmt-edt-wdt", "read-only-access", "array-shape-validation", "atomic-single-file-import", "hash-idempotency"),
    navigation_entries=(
...
RESULT_MIGRATION_MANIFEST = ModuleManifest(
    ...
    dependencies=("core", "auth"), capabilities=("legacy-source-discovery", "legacy-directory-dialog", "pdt-dat", "binary-parser", "orphan-results", "read-only-staging", "atomic-single-file-import", "hash-idempotency"),
    navigation_entries=(
...
def registered_manifests() -> tuple[ModuleManifest, ...]:
    # 测试扩展使用相同契约接入，但 ``extensions.py`` 会按构建类型限制发现，
    # 从而保证生产包中的模块集合保持静态。
...
def validate_manifests(manifests: tuple[ModuleManifest, ...] | list[ModuleManifest]) -> None:
    # 在构建或启动阶段拒绝重复路由、权限和导航键，
    # 避免把错误拖到运行期并表现为与角色相关的不一致行为。
    keys: set[str] = set()
    routes: set[str] = set()
    permissions: set[str] = set()
    navigation_keys: set[str] = set()
    allowed_groups = {"workspace", "methods", "analysis-tests", "data", "tools", "system", "help"}
