...
type Page = 'workspace' | 'methods' | 'migration' | 'spectrum-migration' | 'result-migration' | 'spectra' | 'samples' | 'acquisition' | 'analysis' | 'reports' | 'settings' | 'about' | 'users' | 'audit'
...
const navItems: Array<{ id: Page; label: string; icon: typeof LayoutDashboard; enabled: boolean; hint: string }> = [
  ...
  { id: 'acquisition', label: '采集', icon: Activity, enabled: false, hint: '将在 S11 启用' },
  { id: 'analysis', label: '分析', icon: BarChart3, enabled: false, hint: '将在 S16 启用' },
...
function WorkspaceApp({ token, user, onLogout }: { token: string; user: AuthUser; onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    void api.settings(token).then(setSettings).catch((error) => setToast(error instanceof Error ? error.message : '无法刷新软件设置'))
  }, [page, token])
  useEffect(() => {
    ...
    <div className="app-shell" data-testid="app-shell" data-theme={settings?.display.theme ?? 'light'} data-density={settings?.display.density ?? 'comfortable'}>
      {health === 'offline' && !loading && !about ? <ErrorPage onRetry={loadData} /> : <><Sidebar page={page} onNavigate={handlePage} health={health} user={user} showStatusBar={settings?.display.show_status_bar !== false} />
        <main className="main-area">
          ...
          {page === 'workspace' && <Workspace token={token} canClearEvents={user.permissions.includes('runtime-events.write')} events={events} currentMethod={currentMethod} onNavigate={handlePage} onEventsChange={setEvents} onToast={setToast} />}
          {page === 'methods' && <MethodsPage token={token} currentUser={user} currentMethod={currentMethod} onCurrentMethodChange={setCurrentMethod} onToast={setToast} />}
          {page === 'migration' && <LegacyMigrationPage token={token} currentUser={user} onToast={setToast} />}
          {page === 'spectrum-migration' && <SpectrumMigrationPage token={token} currentUser={user} onToast={setToast} />}
          {page === 'result-migration' && <ResultMigrationPage token={token} currentUser={user} onToast={setToast} />}
          {page === 'spectra' && <SpectrumViewerPage token={token} onToast={setToast} />}
          {page === 'samples' && <SampleQueuePage token={token} onToast={setToast} />}
          ...
          {page === 'users' && <UsersPage token={token} currentUser={user} onToast={setToast} />}
          {page === 'audit' && <AuditPage token={token} />}
          {page !== 'workspace' && page !== 'methods' && page !== 'migration' && page !== 'spectrum-migration' && page !== 'result-migration' && page !== 'spectra' && page !== 'samples' && page !== 'settings' && page !== 'about' && page !== 'users' && page !== 'audit' && <DisabledPage item={navItems.find((item) => item.id === page)!} />}
        </main></>}
      {toast && <div className="toast" role="status"><CheckCircle2 size={16} />{toast}<button title="关闭" onClick={() => setToast(null)}><X size={14} /></button></div>}
    </div>
  )
}
...
function Workspace({ token, canClearEvents, events, currentMethod, onNavigate, onEventsChange, onToast }: { token: string; canClearEvents: boolean; events: RuntimeEvent[]; currentMethod: CurrentMethodState | null; onNavigate: (page: Page) => void; onEventsChange: (events: RuntimeEvent[]) => void; onToast: (message: string) => void }) {
  ...
  return <div className="page-content workspace-page" data-testid="workspace">
    <section className="hero-row"><div><span className="eyebrow"><span className="eyebrow-line" />S07 · SAMPLE QUEUE</span><h1>分析工作台</h1><p>管理方法版本、样品队列与 SpecDirect 旧数据的只读迁移。</p></div><div className="hero-actions"><span className="live-pill"><span className="pulse" />本地运行</span><button className="secondary-button" onClick={() => onNavigate('samples')}><TestTube2 size={16} />样品队列</button></div></section>
    <section className="stat-grid">
      <StatCard icon={Database} label="SQLite 状态" value="WAL" note="外键已启用" tone="blue" />
      <StatCard icon={ShieldCheck} label="注册模块" value="5" note="清单验证通过" tone="green" />
    ...
    <section className="workspace-grid">
      <div className="primary-column">
        <section className="surface status-surface"><div className="surface-heading"><div><span className="section-kicker">CURRENT METHOD</span><h2>{currentMethod?.title || '尚未选择运行方法'}</h2></div><span className={`ready-badge ${hasCurrentMethod ? '' : 'pending'}`}>{hasCurrentMethod ? <CheckCircle2 size={14} /> : <Clock3 size={14} />}{hasCurrentMethod ? `版本 ${currentMethod?.version}` : '等待选择'}</span></div><div className="readiness-grid"><ReadinessItem icon={Database} title="数据存储" detail="SQLite schema v7" done /><ReadinessItem icon={SquareTerminal} title="API 服务" detail="FastAPI /api/v1" done /><ReadinessItem icon={Archive} title="模块清单" detail="5 个模块已注册" done /><ReadinessItem icon={SlidersHorizontal} title="运行方法" detail={hasCurrentMethod ? `${currentMethod?.work_type} · 已发布` : '打开已发布方法后就绪'} done={hasCurrentMethod} /></div><div className="surface-note"><Info size={16} /><span>方法草稿可保留字段错误；只有验证通过并发布的不可变版本才能成为当前运行方法。</span></div></section>
...
function UsersPage({ token, currentUser, onToast }: { token: string; currentUser: AuthUser; onToast: (message: string) => void }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setUsers(nextUsers)
      setRoles(nextRoles)
      if (!roleId && nextRoles[0]) setRoleId(String(nextRoles[0].id))
    } catch (error) { onToast(error instanceof Error ? error.message : '无法读取用户和角色') }
  }, [onToast, roleId, token])
...
function MethodsPage({ token, currentUser, currentMethod, onCurrentMethodChange, onToast }: { token: string; currentUser: AuthUser; currentMethod: CurrentMethodState | null; onCurrentMethodChange: (method: CurrentMethodState) => void; onToast: (message: string) => void }) {
  ...
  return <div className="page-content methods-page" data-testid="methods-page">
    ...
    <div className="method-workbench">
      <aside className="surface method-browser"><div className="surface-heading"><div><span className="section-kicker">METHODS</span><h2>方法列表 <span className="count-badge">{methods.length}</span></h2></div><SlidersHorizontal size={17} /></div><div className="method-list">{methods.length === 0 && <div className="empty-state"><FileText size={24} /><span>暂无方法</span><small>新建第一个方法草稿</small></div>}{methods.map((item) => <button key={item.id} className={`method-list-item ${item.id === selectedId ? 'active' : ''}`} onClick={() => setSelectedId(item.id)}><span className={`method-state-dot ${item.status}`} /><span><strong>{item.name}</strong><small>{item.work_type} · {item.current_version ? `已发布 v${item.current_version}` : '仅草稿'} · 最新 v{item.latest_version}</small></span>{item.is_current && <span className="current-chip">当前</span>}</button>)}</div>{selected && <div className="method-browser-footer"><div><span>状态</span><strong>{selected.status === 'active' ? '启用' : selected.status === 'paused' ? '暂停' : '已删除'}</strong></div><div><span>已发布版本</span><strong>{selected.current_version ? `v${selected.current_version}` : '—'}</strong></div><div><span>最新内容摘要</span><code>{selectedVersion?.content_sha256.slice(0, 10) ?? '—'}</code></div></div>}</aside>
...
function MethodPrintPanel({ methodId, methodName, version, token, canWrite, onToast }: { methodId: number; methodName: string; version: number | null; token: string; canWrite: boolean; onToast: (message: string) => void }) {
  ...
  const submitPrint = async () => {
    ...
    try {
      ...
      setJobs((previous) => [job, ...previous.filter((item) => item.id !== job.id)].slice(0, 8))
  ...
  return <div className="print-workbench" data-testid="method-print-panel">
    ...
    <main className="surface print-preview-panel">
      ...
      <div className="print-jobs"><div className="print-jobs-heading"><strong>最近打印任务</strong><span>失败任务会保留渲染输入和错误 PDF</span></div>{jobs.length === 0 ? <div className="empty-job">尚无打印记录</div> : jobs.slice(0, 5).map((job) => <div className={`print-job ${job.status}`} key={job.id}><span className="job-status">{({ completed: '已完成', queued: '已排队', rendered: '已渲染', failed: '失败' } as Record<string, string>)[job.status]}</span><div><strong>{job.printer_name}</strong><small>v{job.method_version} · {job.page_count} 页 · {formatDateTime(job.created_at)}</small></div><code title={job.output_path ?? job.error_code ?? job.id.slice(0, 10)}>{job.output_path ?? job.error_code ?? job.id.slice(0, 10)}</code></div>)}</div>
    </main>
  </div>
}
...
const defaultLine = (): SpectralLineInput => ({
  ...
  scan_width_points: 9, background_offset_points: 0, peak_mode: 'maximum', peak_width_points: 1,
  fit_mode: 'linear', coordinate_type: 'linear', unit: 'ug/g', value_kind: 'content',
...
function SpectralLinesPanel({ methodId, token, canWrite, onChanged, onToast }: { methodId: number; token: string; canWrite: boolean; onChanged: () => Promise<void>; onToast: (message: string) => void }) {
  ...
  const alignmentLines = collection?.lines.filter((line) => ['alignment', 'internal_standard'].includes(line.line_type) && line.enabled && line.id !== selectedId) ?? []
  ...
  return <div className="spectral-workbench" data-testid="spectral-lines-panel">
    ...
    <section className="surface spectral-list-surface">
      ...
      <div className="spectral-line-list">{collection?.lines.map((line) => <button key={line.id} className={`spectral-line-card ${line.id === selectedId && !creating ? 'active' : ''} ${line.enabled ? '' : 'disabled'}`} onClick={() => edit(line)}>
        ...
        <span className="line-main"><strong>{line.element} <b>{line.wavelength_nm.toFixed(4)}</b> nm</strong><small>{lineTypeLabel[line.line_type]} · {line.detectability?.detectable ? `${line.detectability.ccd_label} / 角度位 ${line.detectability.angle_slot}` : line.detectability?.message}</small></span>
    ...
    <section className="surface spectral-editor-surface">
      ...
      {!selected && !creating ? <div className="method-empty compact"><SlidersHorizontal size={25} /><p>选择一条谱线查看完整参数。</p></div> : <>
        ...
        <div className={`detectability-card ${detectability?.detectable ? 'detectable' : 'undetectable'}`}><span className="detect-icon">{detectability?.detectable ? <CheckCircle2 size={18} /> : <AlertTriangle size={18} />}</span><div><strong>{detectability?.detectable ? '当前条件可检测' : '当前条件不可检测'}</strong><span>{detectability?.detectable ? `${detectability.ccd_label} · 点位 ${detectability.point_index} · 角度位 ${detectability.angle_slot}${detectability.angle_deg == null ? '' : ` (${detectability.angle_deg}°)`}` : detectability?.message ?? '正在计算 CCD 落点…'}</span></div><code>{detectability?.reason_code ?? 'checking'}</code></div>
        ...
        <div className="spectral-form-grid dense">
          ...
          <label className="field"><span>峰值方式</span><select value={draft.peak_mode} onChange={(event) => setDraft((previous) => ({ ...previous, peak_mode: event.target.value as SpectralLineInput['peak_mode'], peak_width_points: event.target.value === 'maximum' ? 1 : Math.max(3, previous.peak_width_points | 1) }))} disabled={!canWrite || isBaseline}><option value="maximum">最大值</option><option value="gaussian">高斯曲线</option></select></label>
          <NumberField label="计算点数" value={draft.peak_width_points} min={1} max={9} step={draft.peak_mode === 'gaussian' ? 2 : 1} disabled={!canWrite || isBaseline || draft.peak_mode === 'maximum'} onChange={(value) => updateDraft('peak_width_points', value)} />
          ...
          <label className="field"><span>拟合坐标</span><select value={draft.coordinate_type} onChange={(event) => updateDraft('coordinate_type', event.target.value as SpectralLineInput['coordinate_type'])} disabled={!canWrite || isBaseline}><option value="linear">普通坐标</option><option value="logarithmic">对数坐标</option></select></label>
...
function LegacyMigrationPage({ token, currentUser, onToast }: { token: string; currentUser: AuthUser; onToast: (message: string) => void }) {
  ...
  return <div className="page-content migration-page" data-testid="legacy-migration-page">
    ...
    {activeRun && <section className="surface migration-report-surface" data-testid="migration-report">
      <div className="surface-heading"><div><span className="section-kicker">MIGRATION REPORT</span><h2>迁移报告 <span className={`migration-status ${activeRun.status}`}>{activeRun.status === 'committed' ? '已提交' : activeRun.status === 'failed' ? '已回滚' : '待提交'}</span></h2></div><code>{activeRun.fingerprint.slice(0, 16)}</code></div>
    ...
    <section className="surface migration-history-surface"><div className="surface-heading"><div><span className="section-kicker">HISTORY</span><h2>最近迁移</h2></div><Archive size={17} /></div>{runs.length === 0 ? <div className="method-empty compact"><Archive size={24} /><p>还没有旧版迁移记录。</p></div> : <div className="migration-history-list">{runs.map((run) => <button key={run.id} onClick={() => openRun(run)} className={activeRun?.id === run.id ? 'active' : ''}><span className={`migration-status ${run.status}`}>{run.status === 'committed' ? '已提交' : run.status === 'failed' ? '已回滚' : '待提交'}</span><div><strong>{run.source_files.mtd?.name ?? 'DIRECT.MTD'}</strong><small>{formatDateTime(run.created_at)} · {run.report.counts.methods} 方法 / {run.report.counts.spectral_lines} 谱线</small></div><code>{run.fingerprint.slice(0, 12)}</code><ChevronRight size={15} /></button>)}</div>}</section>
  </div>
}
...
function LegacySpectrumViewerPage({ token, onToast }: { token: string; onToast: (message: string) => void }) {
  ...
  return <div className="page-content spectrum-viewer-page" data-testid="spectrum-viewer-page">
    ...
    <div className="spectrum-viewer-layout">
      <aside className="surface spectrum-records"><div className="surface-heading"><div><span className="section-kicker">PUBLISHED DATA</span><h2>谱图记录</h2></div><Database size={17} /></div>{records.length === 0 ? <div className="method-empty compact"><Database size={25} /><p>暂无已提交谱图</p></div> : <div className="spectrum-record-list">{records.map((record) => <button key={record.id} className={record.id === selectedId ? 'active' : ''} onClick={() => selectRecord(record)}><span className={`spectrum-kind ${record.kind}`}>{record.kind === 'raw' ? 'RAW' : record.format.toUpperCase()}</span><div><strong>{record.sample_name || record.band_name || record.id}</strong><small>{record.kind === 'raw' ? `${record.ccd_count} CCD · ${record.points_per_ccd} 点` : `${record.sample_count} 样品 · ${record.line_count} 谱线`}</small></div><ChevronRight size={14} /></button>)}</div>}</aside>
      <section className="surface spectrum-workbench">
        {!detail ? <div className="method-empty"><Activity size={30} /><h2>选择一条谱图记录</h2><p>左侧记录会显示 S08/S09 已提交的数据。</p></div> : <>
          ...
          <div className="spectrum-facts"><div><span>来源 SHA-256</span><code>{detail.source_sha256.slice(0, 18)}…</code></div><div><span>样品</span><strong>{detail.sample_name || detail.sample_names?.join(' / ') || '—'}</strong></div><div><span>测量时间</span><strong>{detail.measure_time || '—'}</strong></div><div><span>点数</span><strong>{points.length}</strong></div>{raw && <div><span>当前帧</span><strong>{frameVisible && detail.frame_detail ? `${detail.frame_detail.phase} #${detail.frame_detail.index + 1}` : '均值'}</strong></div>}</div>
        </>}
      </section>
    </div>
  </div>
...
function SpectrumViewerPage({ token, onToast }: { token: string; onToast: (message: string) => void }) {
  ...
  return <div className="page-content spectrum-viewer-page" data-testid="spectrum-viewer-page">
    ...
    <div className="spectrum-viewer-layout">
      <aside className="surface spectrum-records"><div className="surface-heading"><div><span className="section-kicker">PUBLISHED DATA</span><h2>谱图记录 <span className="count-badge">{visibleRecords.length}</span></h2></div><Database size={17} /></div><label className="spectrum-angle-filter"><span>转角</span><select value={angleFilter} onChange={(event) => setAngleFilter(event.target.value)}><option value="all">全部（含未记录）</option>{angles.map((angle) => <option value={String(angle)} key={angle}>{angle}°</option>)}</select></label><div className="spectrum-record-list">{visibleRecords.map((record) => <div className={`spectrum-record-row ${record.id === selectedId ? 'active' : ''}`} key={record.id}><label className="spectrum-overlay-check" title="加入多样品叠加"><input type="checkbox" checked={displayIds.includes(record.id)} disabled={record.kind !== 'raw'} onChange={() => toggleOverlay(record.id)} /></label><button onClick={() => selectRecord(record)}><span className={`spectrum-kind ${record.kind}`}>{record.kind === 'raw' ? 'RAW' : record.format.toUpperCase()}</span><div><strong>{record.sample_name || record.band_name || record.id}</strong><small>{record.kind === 'raw' ? `${record.ccd_count} CCD · ${record.points_per_ccd} 点 · ${record.angle_deg == null ? '转角未记录' : `${record.angle_deg}°`}` : `${record.sample_count} 样品 · ${record.line_count} 谱线`}</small></div><ChevronRight size={14} /></button></div>)}</div><small className="spectrum-overlay-note">可叠加 1–8 个原始样品；点击图例设置优先曲线。</small></aside>
      <section className="surface spectrum-workbench">{!active ? <div className="method-empty"><Activity size={30} /><h2>选择一条谱图记录</h2></div> : <>
        ...
        <div className="spectrum-plot-wrap"><SpectrumPlot curves={curves} xStart={xStart} xEnd={xEnd} yStart={yStart} yEnd={yEnd} tool={tool} cursor={cursor} locked={locked} onCursor={setCursor} onToggleLock={() => setLocked((value) => !value)} onPan={(dx, dy) => { setPanX((value) => Math.max(-1, Math.min(1, value - dx * 2))); setPanY((value) => Math.max(-1, Math.min(1, value + dy * 2))) }} onBoxSelect={boxSelect} />{cursor && <div className="spectrum-cursor-readout"><span>{cursor.curveLabel}</span><strong>{cursor.datum.x.toFixed(3)}</strong><span>点 {cursor.datum.point.point_index} · 强度 {cursor.datum.y.toFixed(3)}</span><span>{locked ? <LockKeyhole size={12} /> : <UnlockKeyhole size={12} />}</span></div>}</div>
