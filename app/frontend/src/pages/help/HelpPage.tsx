...
export function HelpPage({ token, onToast }: { token: string; onToast: (message: string) => void }) {
  ...
  return <div className="page-content help-page refined-page remaining-page" data-testid="help-page">
    <div className="page-intro help-intro">
      <div><h1>帮助中心</h1><p>离线帮助覆盖导航、核心流程、错误码、兼容性、设备安全与维护。</p></div>
    ...
    <TaskLayout className="help-grid" hasItems={Boolean(topics.length)} current={selected?.title ?? "没有匹配主题"}>
      <section className="surface help-index">
        <div className="surface-heading"><div><h2>主题目录</h2></div><div className="help-index-meta"><span>{topics.length} 项</span><BookOpen size={17} /></div></div>
