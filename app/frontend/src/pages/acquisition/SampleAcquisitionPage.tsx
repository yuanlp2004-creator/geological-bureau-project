...
import { NumericInput, reportInvalidNumericInput } from '../../components/NumericInput'
...
export function SampleAcquisitionPage({ token, canWrite, canExecute, onToast }: Props) {
  ...
  const [taskName, setTaskName] = useState('样品采集')
  ...
  const create = async (event: FormEvent) => {
    ...
    try {
      ...
      setCreateError(error instanceof Error ? error.message : '创建采集任务失败'); onToast(error instanceof Error ? error.message : '创建采集任务失败')
    } finally {
      setBusy(false)
    }
  }
    ...
    <TaskLayout className="sample-acquisition-grid" hasItems={Boolean(tasks.length)} current={selected ? `${selected.name} · ${stateLabel[selected.status] ?? selected.status}` : "尚未选择"}>
      ...
      <main className="sample-acquisition-main">
        ...
        <section className="surface acquisition-monitor">
          ...
          {selected ? <>
            <div className="runtime-source">{selected.profile.transport === "simulator" ? "模拟设备 · 仅用于软件验证" : "真实设备 · 连接与协议状态以当前任务反馈为准"}</div><div className="acquisition-progress">
