...
function CurveChart({ snapshot }: { snapshot: AnalysisCurveSnapshot | undefined }) {
  ...
  return <div className="analysis-curve-chart simple-chart-plot-host"><div className="simple-chart-plot"><svg viewBox="40 40 820 260" preserveAspectRatio="none" role="img" aria-label="标准曲线"><rect x="40" y="40" width="820" height="260" /><polyline points={snapshot.chart.map((item) => point(item.intensity, item.value)).join(' ')} />{snapshot.diagnostics.points.map((item) => { const [cx, cy] = point(item.adjusted_intensity ?? 0, item.standard_value).split(','); return <circle key={item.point_index} cx={cx} cy={cy} r="5" /> })}</svg></div><SimpleChartAxes xMin={minX} xMax={maxX} yMin={minY} yMax={maxY} xLabel="强度" yLabel="分析值" /></div>
}
...
export function AnalysisPage({ token, initialView, onViewChange, canExecute, canIntervene, canQuality, canCurve, canPrint, onToast }: Props) {
  ...
  const [name, setName] = useState('定量与曲线分析')
  ...
  const create = async (event: FormEvent) => {
    ...
    if (busy) return
    const reject = (message: string) => { setCreateError(message); onToast(message) }
    if (!canExecute) return reject('当前账户没有执行分析的权限')
    if (!reportInvalidNumericInput(event.currentTarget)) return reject('请先修正慢进超时（秒）的无效数值')
    if (!sampleIds.length) return reject('请至少选择一个已完成采集样品')
    setCreateError(null)
    setBusy(true)
    try {
    ...
    } catch (error) { reject(error instanceof Error ? error.message : '创建分析运行失败') }
    finally { setBusy(false) }
  }
  ...
  return <div className="page-content analysis-page refined-page remaining-page" data-navigation-view={currentView}>
    <section className="analysis-hero"><div><h1>定量分析、重复质控与标准曲线</h1><p>从原始信号到不可变曲线快照、样品结果和合并保存，全程可重放与审计。</p></div><button className="secondary-button" onClick={() => void load()} disabled={busy}><RefreshCw size={15} />刷新</button></section>
    <TaskLayout className="analysis-layout" hasItems={Boolean(runs.length)} current={selected ? `${selected.name} · ${statusLabels[selected.status] ?? selected.status}` : "尚未选择"}>
      <aside className="surface analysis-sidebar"><div className="surface-heading"><div><h2>分析批次</h2></div><BarChart3 size={17} /></div>
      ...
      <main className="analysis-main">
        <FormDisclosure title="建立分析运行" error={createError} hasItems={Boolean(runs.length)} objectKey={selected?.id}><form className="surface analysis-create" onSubmit={create} noValidate><div className="surface-heading"><div><h2>建立分析运行</h2></div><ListChecks size={17} /></div>
          ...
          <button type="submit" className="primary-button" disabled={busy || !canExecute}>{busy ? '处理中…' : '建立并锁定输入'}</button>
        </form></FormDisclosure>
