...
export function AcquisitionPage({ token, canWrite, canExecute, onToast }: AcquisitionPageProps) {
  ...
  const start = async () => {
    ...
    if (!reportInvalidNumericInput(document.querySelector('.debug-toolbar'))) return onToast('请先修正调试参数')
    try {
  ...
  return <div className="page-content acquisition-page refined-page remaining-page" data-testid="acquisition-page">
    <section className="hero-row compact-hero"><div><h1>设备与实时调试</h1><p>使用确定性 ACQ 模拟器观察 CCD 曲线、峰值和连接状态。调试流不会创建样品或谱图记录。</p></div><button className="secondary-button" onClick={() => void load()} disabled={loading}><RefreshCw size={15} className={loading ? 'spin' : ''} />刷新诊断</button></section>
    <div className="acquisition-layout">
      <section className="surface acquisition-sidebar">
        <div className="surface-heading"><div><h2>设备档案</h2></div><SlidersHorizontal size={17} /></div>
        <label className="field"><span>待连接设备档案</span><select value={selectedProfile ?? ''} onChange={(e) => setSelectedProfile(Number(e.target.value))} disabled={!diagnostics?.profiles.length}>{diagnostics?.profiles.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
        {profile && <FormDisclosure title="档案参数与屏幕换算"><div className="device-facts"><div><span>传输</span><strong>{profile.transport === 'simulator' ? 'ACQ 模拟器' : '串口设备'}</strong></div><div><span>串口 / 波特率</span><strong>COM {profile.port} / {profile.baud_rate.toLocaleString()}</strong></div><div><span>CCD 布局</span><strong>{profile.frame_count} 组 × {profile.ccds_per_frame} / {profile.points_per_ccd} 点</strong></div><div><span>保护时间</span><strong>{profile.protection_time_ms} ms</strong></div><div><span>屏幕换算</span><strong>{conversion ? `${conversion.pixels_per_mm.toFixed(2)} px/mm` : '--'}</strong></div></div></FormDisclosure>}
        {profile && <span className="device-source">{profile.transport === "simulator" ? "模拟设备" : "真实串口设备"}</span>}<div className="device-actions"><button className="primary-button" onClick={() => void connect()} disabled={!canExecute || diagnostics?.adapter.connected}><Cable size={15} />连接诊断</button><button className="secondary-button" onClick={() => void disconnect()} disabled={!canExecute || !diagnostics?.adapter.connected}><XCircle size={15} />断开</button></div>
