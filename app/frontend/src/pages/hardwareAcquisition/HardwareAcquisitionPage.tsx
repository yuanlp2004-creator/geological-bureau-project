...
export function HardwareAcquisitionPage({ token, canWrite, canExecute, onToast }: Props) {
  ...
  const [name, setName] = useState('自动转角任务')
  ...
  const create = async (event: FormEvent) => {
    ...
    try {
      ...
      setCreateError(error instanceof Error ? error.message : '转角 JSON 无法解析'); onToast(error instanceof Error ? error.message : '转角 JSON 无法解析')
      return
    }
    setCreateError(null)
    setBusy(true)
    try {
      ...
      setCreateError(error instanceof Error ? error.message : '创建自动转角任务失败'); onToast(error instanceof Error ? error.message : '创建自动转角任务失败')
    } finally {
      setBusy(false)
    }
  }
    ...
    <TaskLayout className="hardware-layout" hasItems={Boolean(tasks.length)} current={selected ? `${selected.name} · ${labels[selected.status] ?? selected.status}` : "尚未选择"}>
      ...
      <main className="hardware-main">
        ...
        <section className="surface hardware-monitor">
          ...
          {selected ? <>
            <div className="runtime-source">{selected.transport === "simulator" ? "模拟设备 · 仅用于软件验证" : "真实串口 · 转角协议缺失时禁止执行"}</div><div className="hardware-progress">
