...
export function MaintenancePage({ token, canWrite, onToast }: { token: string; canWrite: boolean; onToast: (message: string) => void }) {
  ...
  useEffect(() => {
    void load()
    void api.settings(token).then((settings) => setDirectory(settings.directories.backups))
      .catch((error) => onToast(error instanceof Error ? error.message : '无法读取备份目录设置'))
  }, [token])
  ...
  return <div className="page-content maintenance-page refined-page remaining-page" data-testid="maintenance-page">
    <div className="page-intro maintenance-intro">
      <div><h1>备份与维护</h1><p>在线备份、隔离恢复演练和受控空间维护。每次操作都会写入维护记录。</p></div>
    ...
    <div className="maintenance-grid">
      <section className="surface maintenance-primary">
        <div className="surface-heading"><div><h2>SQLite 在线备份</h2></div><DatabaseBackup size={18} /></div>
        ...
        <div className="maintenance-action-row">
          ...
          <button className="secondary-button danger" disabled={!canWrite || busy} onClick={() => void run(() => api.maintenanceAction(token, 'retention'), '备份保留策略已执行')}><Wrench size={15} />清理过期备份</button>
        </div>
      ...
      <section className="surface maintenance-status">
        <div className="surface-heading"><div><h2>数据库状态</h2></div><span className={`state-chip ${status?.integrity === 'ok' ? 'completed' : 'failed'}`}>{status?.integrity ?? '读取中'}</span></div>
        ...
        <div className="maintenance-action-group"><span className="maintenance-group-label">数据库整理</span><div className="maintenance-action-row">
          <button className="secondary-button danger" disabled={!canWrite || busy} onClick={() => void run(() => api.maintenanceAction(token, 'checkpoint', { mode: 'TRUNCATE' }), 'WAL checkpoint 已完成')}><CheckCircle2 size={15} />WAL checkpoint</button>
          <button className="secondary-button danger" disabled={!canWrite || busy} onClick={() => void run(() => api.maintenanceAction(token, 'optimize'), '数据库优化已完成')}>优化</button>
          <button className="secondary-button danger" disabled={!canWrite || busy} onClick={() => void run(() => api.maintenanceAction(token, 'reclaim'), '空间回收已完成')}>受控回收</button>
        </div></div>
        <div className="maintenance-action-group"><div className="maintenance-retention-heading"><span className="maintenance-group-label">文件清理</span><label className="maintenance-retention-field"><span>保留天数</span><NumericInput min={1} max={3650} value={retentionDays} onValueChange={setRetentionDays} /></label></div><div className="maintenance-action-row">
          <button className="secondary-button danger" disabled={!canWrite || busy} onClick={() => void run(() => api.maintenanceAction(token, 'logs/cleanup', { retention_days: retentionDays }), '过期轮转日志已清理')}>清理过期日志</button>
          <button className="secondary-button danger" disabled={!canWrite || busy} onClick={() => void run(() => api.maintenanceAction(token, 'temp/cleanup', { retention_days: retentionDays }), '过期临时文件已清理')}>清理临时文件</button>
        </div></div>
      </section>
    </div>
    <section className="surface maintenance-table">
      <div className="surface-heading"><div><h2>备份记录</h2></div><span className="muted-label">{status?.backups.length ?? 0} 条</span></div>
      {!status?.backups.length ? <div className="maintenance-empty"><DatabaseBackup size={22} /><strong>尚无备份记录</strong><span>创建在线备份后，路径、哈希和恢复演练入口会显示在这里。</span></div> : <div className="maintenance-list">{status.backups.map((backup) => <div className="maintenance-row" key={backup.id}><div><strong title={backup.backup_path}>{backup.backup_path}</strong><small>{backup.created_at} · {(backup.byte_length / 1024).toFixed(1)} KB · SHA-256 <CopyableCode value={backup.backup_sha256} visibleLength={16} /></small></div><span className="state-chip completed">{backup.status}</span><div className="maintenance-row-actions"><button className="secondary-button compact-button" disabled={!canWrite || busy} onClick={() => void run(() => api.verifyBackup(token, backup.id), '备份校验通过')}>校验</button><button className="secondary-button compact-button" disabled={!canWrite || busy} onClick={() => void run(() => api.restoreRehearsal(token, backup.id), '恢复演练通过')}>恢复演练</button></div></div>)}</div>}
    </section>
    <section className="surface maintenance-history">
      <div className="surface-heading"><div><h2>最近维护记录</h2></div><span className="muted-label">{status?.operations.length ?? 0} 条</span></div>
